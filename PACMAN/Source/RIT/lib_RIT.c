...
uint32_t init_RIT ( uint32_t RITInterval )
{
  ...
  LPC_SC->PCLKSEL1  |=  (1<<26);   // RIT Clock = CCLK
	...
	//NVIC_SetPriority(RIT_IRQn, 1); //mettendo questo il rit ha meno priorità
  return (0);
}

uint32_t read_RIT(void) {
    return LPC_RIT->RICOUNTER;  // Restituisce il valore corrente del conteggio RIT
}
