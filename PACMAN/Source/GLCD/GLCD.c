...
void LCD_DrawEmptySquare( uint16_t x0, uint16_t y0, uint16_t l, uint16_t h , uint16_t color ){
	LCD_DrawLine(x0,y0,x0+l,y0,color);
	LCD_DrawLine(x0,y0,x0,y0+h,color);
	LCD_DrawLine(x0,y0+h,x0+l,y0+h,color);
	LCD_DrawLine(x0+l,y0,x0+l,y0+h,color);
}

void LCD_DrawFullSquare( uint16_t x0, uint16_t y0, uint16_t d, uint16_t color){
	int i,j;
	int b=(L-d)/2; //border
	for (i=0;i<d;i++){
		for(j=0;j<d;j++){
			LCD_SetPoint(x0+b+j,y0+b+i,color);
		}
	}
}

void LCD_DrawPacman( uint16_t x0, uint16_t y0, uint16_t d, uint16_t color){
	int i,j;
	int b=(L-d)/2; //border
	for (i=0;i<d;i++){
		for(j=0;j<d;j++){
			if(((j==0)&&(i==0))||((j==0)&&(i==(d-1)))||((j==(d-1))&&(i==0))||((j==(d-1))&&(i==(d-1)))){
...
void LCD_DrawGhost( uint16_t x0, uint16_t y0, uint16_t color){
	int i,j;
	LCD_SetPoint(x0+5,y0+2,color);
	for(i=3;i<=7;i++){
		LCD_SetPoint(x0+i,y0+3,color);
	}
	for(i=2;i<=8;i++){
		for(j=4;j<=8;j++){
